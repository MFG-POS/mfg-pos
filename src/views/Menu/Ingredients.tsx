...
import { deleteIngredientContent, deleteIngredientHeader, deleteIngredientToast } from 'others/modal-messages';
...
const Ingredients = () => {
  ...
  const onRowDelete = (row: Row<TableIngredient>): void => {
    deleteDoc('ingredients', row.original.id).then(() => stateRefresher(!state));
  };
  ...
  const actions: TableAction<TableIngredient>[] = useMemo(
    () => [
      { name: 'Edytuj', callback: onRowEdit },
      {
        ...
        modalHeader: deleteIngredientHeader,
        modalContent: deleteIngredientContent,
        modalToast: deleteIngredientToast,
      },
    ],
    [],
  );
